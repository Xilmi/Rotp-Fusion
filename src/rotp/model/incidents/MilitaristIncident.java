...
public class MilitaristIncident extends DiplomaticIncident {
    private static final long serialVersionUID = 1L;
    final int empYou;
    final int empMe;
    float severityGoal;
    EmpireView iev;
    public static MilitaristIncident create(EmpireView ev) {
        return new MilitaristIncident(ev);
    }
    @Override
    ...
    public void update()
    {
        ...
        if(iev.owner().generalAI().absolution() < 1 || !iev.owner().leader().isMilitarist())
        {
            severity = 0;
            return;
        }
    ...
    @Override
    public String title()            { return text("INC_MILITARIST_TITLE"); }
    @Override
    public String description()      { 
        if(severity < 0)
            return decode(text("INC_MILITARIST_DESC1"));
        else
            return decode(text("INC_MILITARIST_DESC2"));
    }
    @Override
    public String key() {
        return "Militarist";
